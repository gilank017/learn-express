...
class Validation {
  async validate(payload, required = [], optional = [], prevent = []) {
    const rules = []
    for(let key in this) {
  ...
  async #check (fields) {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      for (let j = 0; j < fields[i].rules.length; j++) {
        if (fields[i].value !== undefined && !Array.isArray(fields[i].value)) {
        ...
        if (validation.exists(fields[i].value) && fields[i].rules[j] == 'datetime') {
          if (!validator.isISO8601(fields[i].value, {strict: true, strictSeparator: true})) {
            messages.push('Invalid Datetime, Not YYYY-MM-DDThh:mm:ss format')
          }
        }

        if (validation.exists(fields[i].value) && fields[i].rules[j] == 'date') {
          if (!validator.isDate(fields[i].value, 'YYYY-MM-DD')) {
            messages.push('Invalid Date, Not YYYY-MM-DD format or out of range')
          }
        }
